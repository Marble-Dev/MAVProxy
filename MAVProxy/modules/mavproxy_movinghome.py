...
class movinghome(mp_module.MPModule):
    ...
    def cmd_movinghome(self, args):
        ...
        if len(args) == 0:
            ...
            print(
                f"Home Point:\nlatitude: {self.lath}, longitude: {self.lonh}, altitude: {self.alth}"
            )
            return
        else:
            print(self.usage)
    ...
    def status(self):
        ...
        if self.updating == True:
            print(
                "Last known GCS position lat %(lat)f lon=%(lon)f  max radius=%(max).1fm"
                % {
                    "lat": self.lath,
                    "lon": self.lonh,
                    "max": self.radius,
    ...
    def movinghome_on(self):
        ...
        print(
            "Home position will be updated if GCS moves > %(max).1fm"
            % {
                "max": self.radius,
            }
        )
        ...
        if self.updating == True:
            ...
            try:
                ...
                if now - self.last_decode_error_print > 10:
                    print(f"movinghome: decode error; baudrate issue? data = {data}")
            ...
            if data.startswith("$GPGGA") or data.startswith("$GNGGA,"):
                ...
                if now - self.last_check > self.check_interval:
                    ...
                    self.dist = self.haversine(
                        self.lon, self.lat, self.alt, self.lonh, self.lath, self.alth
                    )
                    print(
                        f"movinghome: distance from home = {self.dist}, radius = {self.radius}"
                    )
                    if self.dist > self.radius:
                        ...
                        print("movinghome: home position updated! ~~~~~~~~~~~~~~~~~~")
